...
void  PID_Init(pid_ctrl_t* pid, float Kp, float Ki, float Kd,
               float Ts_sec, float out_min, float out_max);
...
float FanMgr_ComputeControlTemp(const float* temps, size_t num_temps,
                                temp_ctrl_method_t method, size_t top_n);

/* High-level helper: compute control temperature then compute PWM using the PID.
   Returns 0..255 (uint8_t). */
uint8_t FanMgr_UpdatePWM(pid_ctrl_t* pid,
                         const float* temps, size_t num_temps,
                         temp_ctrl_method_t method, size_t top_n);

/* Utility: hard clamp to [0,255] and sanitize NaNs/Infs. */
static inline uint8_t FanMgr_ClampPWM(float u) {
    if (!(u == u)) return 0; // NaN -> 0
    if (u < 0.0f)  return 0;
